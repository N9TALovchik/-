...
function ShopManager:Init(Window, Tabs)
    ...
    -- =====================================================
    -- ADMIN ESP (показывает "Admin" над игроками с тегом "Trusted")
    -- =====================================================
    local adminGroup = shopTab:AddRightGroupbox('Admin ESP')
    local adminToggle = adminGroup:AddToggle('AdminESPEnabled', {
        Text = 'Enable Admin ESP',
        Default = false,
        Tooltip = 'Показывает "Admin" над игроками с тегом Trusted'
    })
    local adminColorPicker = adminGroup:AddLabel('Text Color'):AddColorPicker('AdminESPColor', {
        Default = Color3.fromRGB(255, 255, 0),
        Title = 'Admin Text Color'
    })

    local adminDrawings = {}  -- player -> Drawing.Text

    local function updateAdminESP()
        if not adminToggle.Value then
            for _, text in pairs(adminDrawings) do
        ...
        -- Размер берём из основного ESP (Name Size)
        local nameSize = (_G.NOTALovchik_v190 and _G.NOTALovchik_v190.Name and _G.NOTALovchik_v190.Name.Size) or 14
        local color = adminColorPicker.Value or Color3.fromRGB(255, 255, 0)
        local players = game:GetService("Players"):GetPlayers()

        for _, plr in ipairs(players) do
            if not plr:HasTag("Trusted") then
                -- если был текст – скрываем
                if adminDrawings[plr] then adminDrawings[plr].Visible = false end
                continue
            end
            ...
            local pos = head and head.Position or (root and root.Position + Vector3.new(0, 2, 0))
            if not pos then
                if adminDrawings[plr] then adminDrawings[plr].Visible = false end
                continue
            end
            ...
            local text = adminDrawings[plr]
            if not text then
                text = Drawing.new("Text")
                text.Center = true
                text.Outline = true
                text.Font = 2  -- GothamBold
                adminDrawings[plr] = text
            end

            text.Text = "Admin"
            text.Size = nameSize
            text.Color = color
            -- Позиция: над головой, выше стандартного Name ESP (смещение вверх)
            local offsetY = nameSize * 1.2
    ...
    local function startAdminLoop()
        ...
        adminConnection = game:GetService("RunService").RenderStepped:Connect(updateAdminESP)
    end
    local function stopAdminLoop()
        if adminConnection then adminConnection:Disconnect() adminConnection = nil end
        for _, text in pairs(adminDrawings) do
            pcall(text.Remove, text)
        end
        adminDrawings = {}
    ...
    adminToggle:OnChanged(function(enabled)
        if enabled then
            startAdminLoop()
        else
            stopAdminLoop()
    ...
    adminColorPicker:OnChanged(function()
        if adminToggle.Value then
            updateAdminESP()
        end
    end)
