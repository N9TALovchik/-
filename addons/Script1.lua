-- ShopManager.lua (полный финальный + Anti-Aim)
local ShopManager = {}
...
checkLoop()
    ...
    -- ANTI-AIM GROUP
    -- =====================================================
    local antiAimGroup = shopTab:AddLeftGroupbox('Anti-Aim')
    
    local antiAimEnabledToggle = antiAimGroup:AddToggle('AntiAimEnabled', {
        Text = 'Enabled',
        Default = false,
        Tooltip = 'Включает подмену поворота камеры'
    })
    
    local yawTypeDropdown = antiAimGroup:AddDropdown('YawType', {
        Text = 'Yaw Type',
        Values = {'Jitter', 'Spin', 'Offset'},
        Default = 'Jitter',
        Multi = false,
        AllowNull = false,
        Tooltip = 'Тип горизонтального поворота'
    })
    
    local pitchTypeDropdown = antiAimGroup:AddDropdown('PitchType', {
        Text = 'Pitch',
        Values = {'Up', 'Down', 'Zero', 'Random'},
        Default = 'Zero',
        Multi = false,
        AllowNull = false,
        Tooltip = 'Направление взгляда по вертикали'
    })
    
    -- Слайдеры для Jitter
    local jitterLeftSlider = antiAimGroup:AddSlider('JitterLeftLimit', {
        Text = 'Jitter Left Limit',
        Min = -180, Max = 180, Default = -90, Rounding = 0, Suffix = '°'
    })
    local jitterRightSlider = antiAimGroup:AddSlider('JitterRightLimit', {
        Text = 'Jitter Right Limit',
        Min = -180, Max = 180, Default = 90, Rounding = 0, Suffix = '°'
    })
    
    -- Слайдер для Spin
    local spinSpeedSlider = antiAimGroup:AddSlider('SpinSpeed', {
        Text = 'Spin Speed',
        Min = 0, Max = 1000, Default = 200, Rounding = 0, Suffix = '°/s'
    })
    
    -- Слайдер для Offset
    local offsetSlider = antiAimGroup:AddSlider('OffsetAngle', {
        Text = 'Offset Angle',
        Min = -180, Max = 180, Default = 0, Rounding = 0, Suffix = '°'
    })
    
    -- Скрытие/показ слайдеров в зависимости от YawType
    local function updateSliderVisibility()
        local yawType = yawTypeDropdown.Value
        jitterLeftSlider.Container.Parent.Visible = (yawType == 'Jitter')
        jitterRightSlider.Container.Parent.Visible = (yawType == 'Jitter')
        spinSpeedSlider.Container.Parent.Visible = (yawType == 'Spin')
        offsetSlider.Container.Parent.Visible = (yawType == 'Offset')
    end
    
    yawTypeDropdown:OnChanged(updateSliderVisibility)
    updateSliderVisibility() -- начальное состояние
    ...
    local jitterSide = 1  -- 1 = right, -1 = left
    local antiAimHeartbeat = nil
    
    -- Получение фейкового направления по вертикали
    local function getPitchLookVector()
        local pitchType = pitchTypeDropdown.Value
        if pitchType == 'Up' then
            return Vector3.new(0, 1, 0)  -- смотрим строго вверх
        elseif pitchType == 'Down' then
            return Vector3.new(0, -1, 0) -- строго вниз
        elseif pitchType == 'Zero' then
            return Vector3.new(0, 0, -1) -- горизонтально (вперёд)
        else -- Random
            local randomY = (math.random() * 2 - 1) -- от -1 до 1
            local horizontalMagnitude = math.sqrt(1 - randomY * randomY)
            local horizontalAngle = math.random() * math.pi * 2
            local x = math.cos(horizontalAngle) * horizontalMagnitude
            local z = math.sin(horizontalAngle) * horizontalMagnitude
            return Vector3.new(x, randomY, z)
        end
    end
    
    -- Получение угла рыскания в зависимости от режима
    local function getYawAngle()
        local yawType = yawTypeDropdown.Value
        if yawType == 'Jitter' then
            local now = os.clock()
            if now - lastJitterTime > 0.3 then  -- переключаем сторону каждые 0.3 сек
                jitterSide = -jitterSide
                lastJitterTime = now
            end
            if jitterSide == 1 then
                return math.rad(jitterRightSlider.Value)
            else
                return math.rad(jitterLeftSlider.Value)
            end
        elseif yawType == 'Spin' then
            spinAngle = spinAngle + math.rad(spinSpeedSlider.Value) * task.wait() -- на самом деле используем deltaTime из Heartbeat
            return spinAngle
        else -- Offset
            return math.rad(offsetSlider.Value)
        end
    end
    
    -- Пересчёт spinAngle с учётом deltaTime (вызывается в Heartbeat)
    local lastUpdateTime = os.clock()
    local function updateSpinAngle()
        local now = os.clock()
        local dt = now - lastUpdateTime
        lastUpdateTime = now
        if yawTypeDropdown.Value == 'Spin' then
            spinAngle = spinAngle + math.rad(spinSpeedSlider.Value) * dt
        end
    end
    
    -- Подмена отправки камеры (серверу)
    local function hookCameraSend()
        if oldCameraSend then return end  -- уже перехвачено
        local replicatedStorage = game:GetService("ReplicatedStorage")
        local packets = require(replicatedStorage:WaitForChild("Network"):WaitForChild("Packets"))
        local replicateCamera = packets.Character.packets.ReplicateCamera
        oldCameraSend = replicateCamera.send
        replicateCamera.send = function(self, cframe)
            if not antiAimActive then
                return oldCameraSend(self, cframe)
            end
            ...
            -- Берём реальную позицию камеры
            local realPos = workspace.CurrentCamera.CFrame.Position
            
            -- Вычисляем фейковое направление взгляда
            local pitchLook = getPitchLookVector()
            local yawAngle = getYawAngle()
            
            -- Поворачиваем направление по горизонтали (Yaw)
            local lookVector = Vector3.new(
                math.cos(yawAngle) * pitchLook.X - math.sin(yawAngle) * pitchLook.Z,
                pitchLook.Y,
                math.sin(yawAngle) * pitchLook.X + math.cos(yawAngle) * pitchLook.Z
            ).Unit
            
            -- Создаём фейковый CFrame
            local fakeCF = CFrame.lookAt(realPos, realPos + lookVector)
            
            -- Локально поворачиваем персонажа
            local rootPart = char:FindFirstChild("HumanoidRootPart")
            if rootPart then
                -- Отключаем AutoRotate, чтобы наш поворот не перезаписывался
                local humanoid = char:FindFirstChild("Humanoid")
                if humanoid then
                    humanoid.AutoRotate = false
                end
                -- Мгновенно поворачиваем RootPart в горизонтальной плоскости
            ...
            return oldCameraSend(self, fakeCF)
        end
    end
    
    -- Восстановление оригинальной отправки камеры
    local function unhookCameraSend()
        if not oldCameraSend then return end
        local replicatedStorage = game:GetService("ReplicatedStorage")
        local packets = require(replicatedStorage:WaitForChild("Network"):WaitForChild("Packets"))
        local replicateCamera = packets.Character.packets.ReplicateCamera
        replicateCamera.send = oldCameraSend
        oldCameraSend = nil
    end
    
    -- Запуск Anti-Aim
    local function startAntiAim()
        if antiAimActive then return end
        antiAimActive = true
        spinAngle = 0
        ...
        -- Включаем цикл для Spin (плавное вращение) и локального поворота
        antiAimHeartbeat = game:GetService("RunService").Heartbeat:Connect(function()
            if not antiAimActive then return end
            updateSpinAngle()
            
            -- Локальный поворот (если режим Jitter или Offset, тоже обновляем)
            local player = game.Players.LocalPlayer
            local char = player.Character
            if not char then return end
            local rootPart = char:FindFirstChild("HumanoidRootPart")
    ...
    local function stopAntiAim()
        ...
        if antiAimHeartbeat then
            antiAimHeartbeat:Disconnect()
            antiAimHeartbeat = nil
        end
    ...
    -- Обработчик переключения тоггла
    antiAimEnabledToggle:OnChanged(function(enabled)
        if enabled then
            startAntiAim()
            Library:Notify("Anti-Aim включён", 2)
        else
            stopAntiAim()
            Library:Notify("Anti-Aim выключен", 2)
        end
    end)
    -- =====================================================
