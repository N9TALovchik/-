...
local SaveManager = {} do
		...
		ColorPicker = {
			Save = function(idx, object)
				if type(object.GetSaveData) == 'function' then
					local d = object:GetSaveData(); d.type = 'ColorPicker'; d.idx = idx; return d
				end
			...
			Load = function(idx, data)
				...
				if type(cp.LoadSaveData) == 'function' then cp:LoadSaveData(data)
				else
	...
	function SaveManager:BuildConfigSection(tab)
		...
		section:AddButton('Export config', function()
			...
			if not success then self.Library:Notify('Export failed: ' .. err, 3) end
		end)
